...
export interface ViewModel {
  ...
  hearGraphTicks: GraphTick[];
  hearRightPath: string;
  hearLeftPath: string;
  hearRightPoints: GraphPoint[];
  hearLeftPoints: GraphPoint[];
  hearRefLineY: number;
  hearDbTicks: DbTick[];
...
function hearGraphX(freq: number): number {
  const logMin = Math.log10(HEAR_AXIS_MIN_FREQ);
  const logMax = Math.log10(HEAR_AXIS_MAX_FREQ);
...
function buildHearPoints(results: HearResults, ear: Ear, listeningScale: number): GraphPoint[] {
  ...
  for (const f of HEARING_FREQS) {
    ...
    pts.push({ freq: f, x: hearGraphX(f), y: hearGraphY(db), noResponse: !!r.noResponse });
  }
  return pts;
}
...
export function computeViewModel(s: AppState): ViewModel {
  ...
  const hearGraphTicks: GraphTick[] = HEARING_FREQS.map((f) => ({
    x: hearGraphX(f),
    label: f >= 1000 ? f / 1000 + 'k' : String(f),
  }));
  const listeningScale = s.hearListeningType === 'listening' ? 0.5 : 1;
  const hearRightPoints = buildHearPoints(s.hearResults, 'right', listeningScale);
  const hearLeftPoints = buildHearPoints(s.hearResults, 'left', listeningScale);
  const hearRightPath = buildHearPath(hearRightPoints);
  const hearLeftPath = buildHearPath(hearLeftPoints);
