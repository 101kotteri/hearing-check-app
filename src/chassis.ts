...
// iPad's own frame: same width/footer (decorative buttons+knobs) as the PC
// chassis at their native size — explicit direction NOT to stretch those —
// but a taller bezel (screen), computed as the PC bezel's nested virtual
// canvas (1440x630, the same coordinate space renderScreenContent's PC
// templates already render into, scaled 0.9 to fit the 1296x567 clip box)
// grown to 1440x819 (630*1.3, matching the requested "1.3x taller" ratio)
// at the same 0.9 scale — everything below the bezel (divider/buttons/knobs)
// just shifts down by the resulting height delta, unchanged in size.
const TABLET_NESTED_CANVAS_W = 1440;
const TABLET_NESTED_CANVAS_H = 630 * 1.3; // 819
const TABLET_NESTED_SCALE = 0.9; // same as PC's — keeps the 1440-wide canvas mapping to the same 1296px clip width
const TABLET_INNER_CLIP_W = TABLET_NESTED_CANVAS_W * TABLET_NESTED_SCALE; // 1296, same as PC
const TABLET_INNER_CLIP_H = TABLET_NESTED_CANVAS_H * TABLET_NESTED_SCALE; // 737.1
const TABLET_BEZEL_PAD = 14;
const TABLET_BEZEL_H = TABLET_INNER_CLIP_H + TABLET_BEZEL_PAD * 2; // 765.1
const TABLET_BEZEL_H_DELTA = TABLET_BEZEL_H - 595; // growth vs. PC's original bezel
export const TABLET_FRAME_W = PC_CHASSIS_W;
export const TABLET_FRAME_H = PC_CHASSIS_H + TABLET_BEZEL_H_DELTA;

export function renderTabletFrame(): string {
  const buttons = Array.from({ length: 12 }, () => `<div style="${BUTTON_STYLE}"></div>`).join('');
  const knobAngles = [-30, 10, 45];
  const knobs = knobAngles
    .map(
  ...
  const w = TABLET_FRAME_W;
  const footerTop = 657 + TABLET_BEZEL_H_DELTA;

  return `
  <div id="app-shell" style="width:${w}px;height:${TABLET_FRAME_H}px;background:linear-gradient(150deg, var(--frame-1) 0%, var(--frame-2) 65%, oklch(0.27 0.06 270) 100%);position:relative;border-radius:26px;box-shadow:0 24px 70px rgba(0,0,0,0.5), inset 0 2px 0 oklch(0.75 0.03 260 / 0.3), inset 0 -8px 16px rgba(0,0,0,0.42), inset 8px 0 16px rgba(0,0,0,0.16), inset -8px 0 16px rgba(0,0,0,0.16);font-family:var(--font-display);">
    <div style="position:absolute;inset:0;background:radial-gradient(ellipse 700px 400px at 18% 0%, oklch(0.5 0.03 240 / 0.22), transparent 60%);border-radius:26px;pointer-events:none;"></div>
    <div style="position:absolute;inset:0;background:radial-gradient(ellipse 480px 260px at 93% 4%, oklch(0.75 0.02 260 / 0.11), transparent 55%);border-radius:26px;pointer-events:none;"></div>
    <div style="position:absolute;inset:0;background:radial-gradient(ellipse 420px 220px at 5% 97%, oklch(0.68 0.02 260 / 0.08), transparent 55%);border-radius:26px;pointer-events:none;"></div>
    <div style="position:absolute;inset:0;border-radius:26px;border:1px solid oklch(0.5 0.02 240 / 0.18);pointer-events:none;"></div>
    ...
    <div style="position:absolute;top:50px;left:90px;width:1324px;height:${TABLET_BEZEL_H}px;background:var(--frame-bezel);border-radius:16px;box-shadow:inset 0 6px 20px rgba(0,0,0,0.9), inset 0 -5px 15px rgba(0,0,0,0.7), inset 6px 0 15px rgba(0,0,0,0.6), inset -6px 0 15px rgba(0,0,0,0.6), 0 1px 0 oklch(0.5 0.03 240 / 0.15), 0 0 28px 8px rgba(0,0,0,0.55);padding:${TABLET_BEZEL_PAD}px;overflow:hidden;">
      <div style="width:${TABLET_INNER_CLIP_W}px;height:${TABLET_INNER_CLIP_H}px;overflow:hidden;border-radius:6px;position:relative;">
        <div style="width:${TABLET_NESTED_CANVAS_W}px;height:${TABLET_NESTED_CANVAS_H}px;transform:scale(${TABLET_NESTED_SCALE});transform-origin:top left;background:var(--bg);color:var(--text);font-family:var(--font-display);position:relative;overflow:hidden;">
          <div style="position:absolute;inset:0;background-image:repeating-linear-gradient(0deg, transparent, transparent 39px, var(--line) 40px), repeating-linear-gradient(90deg, transparent, transparent 39px, var(--line) 40px);opacity:0.2;pointer-events:none;"></div>
    ...
    <div style="position:absolute;top:${footerTop}px;left:277px;width:950px;height:1px;background:oklch(0.5 0.02 240 / 0.25);pointer-events:none;"></div>

    <div style="position:absolute;top:${footerTop + 20}px;left:0;width:${w}px;display:flex;justify-content:center;gap:9px;pointer-events:none;">${buttons}</div>

    <div style="position:absolute;top:${footerTop + 57}px;left:0;width:${w}px;display:flex;justify-content:center;align-items:center;gap:60px;pointer-events:none;">${knobs}</div>
  </div>`;
}
